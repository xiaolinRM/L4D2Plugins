...
public OnPluginStart()
{
    ...
    HookConVarChange(g_hMode, OnConVarChanged);
    HookConVarChange(g_hTeam, OnConVarChanged);
    HookConVarChange(g_hType, OnConVarChanged);
    UpdateConVarValue();
...
public Action:CommandListener_KickID(client, const String:command[], argc)
{
    if (client || !argc) return Plugin_Continue;
    new String:args[64];
    GetCmdArgString(args, sizeof(args));
    if (StrContains(args, "CBaseEntity::InputKill()") == -1) return Plugin_Continue;
    new target = GetClientOfUserId(StringToInt(args));
    if (!target || !IsClientConnected(target)) return Plugin_Continue;
    new bool:bTeam = IsClientInGame(target) && (g_iTeam & (1 << (GetClientTeam(target) - 1)));
    new bool:bType = (g_iType & (IsFakeClient(target) ? 2 : 1)) != 0;
    return (g_bMode ? (bTeam && bType) : (bTeam || bType)) ? Plugin_Stop : Plugin_Continue;
}

UpdateConVarValue()
{
    g_bMode = GetConVarBool(g_hMode);
    g_iTeam = GetConVarInt(g_hTeam);
